...
namespace MiniProyecto3.controlador
{
    public class GimnasioMaster
    {
        ...
        public void consultarParticipantesInscritos(frmParticipantesInscritos r)
        {
            SqlDataReader lector = gg.consultaParticipantesInscritos(int.Parse(r.cmbClase.Text));
            DataTable dt = new DataTable();
            dt.Load(lector);
            r.dgvParticipantes.DataSource = dt;
        }

        public void consultarDetallesAfiliacion(frmDetallesAfiliaciones r)
        {
            SqlDataReader lector = gg.consultaDetallesAfiliacion(r.dtpFechaInicio.Value.ToShortDateString(), r.dtpFechaFin.Value.ToShortDateString());
            DataTable dt = new DataTable();
            dt.Load(lector);
            r.dgvDetalles.DataSource = dt;
        }
        public void consultarClientesActivos(frmClientesActivos r)
        {
            SqlDataReader lector = gg.consultaClientesActivos();
            DataTable dt = new DataTable();
            dt.Load(lector);
            r.dgvClientes.DataSource = dt;
        }
        ...
        public void listaIdClases(frmParticipantesInscritos r)
        {
            SqlDataReader lector = gg.listadoIdClases();
            DataTable dt = new DataTable();
            dt.Columns.Add("id_clase", typeof(int));
            dt.Load(lector);
            r.cmbClase.ValueMember = "id_clase";
            r.cmbClase.DataSource = dt;
        }
